#!/usr/bin/env python3
"""
Enhanced Test Base Class with Selective Cleanup System
...
class EnhancedTestBase:
    """Enhanced base class with selective cleanup - preserves failed test data for debugging"""
    ...
    def __init__(self, base_url="https://chroma-load-balancer.onrender.com", test_prefix="AUTOTEST"):
        ...
        logger.info(f"🧹 Enhanced selective cleanup system enabled")
    ...
    def log_test_result(self, test_name, success, details="", duration=0):
        ...
        # Plan cleanup action
        if success:
            logger.debug(f"📋 Test {test_name} passed - data will be cleaned up")
        else:
    ...
    def selective_cleanup(self):
        """Selective cleanup - only clean up data from successful tests"""
        logger.info("🧹 Performing selective test data cleanup...")
        logger.info("   ✅ Cleaning data from PASSED tests only")
        ...
        logger.info(f"🧹 Selective Cleanup Summary:")
    ...
    def comprehensive_cleanup(self):
        """Fallback comprehensive cleanup (cleans everything)"""
        logger.warning("🧹 Performing COMPREHENSIVE cleanup (all data will be removed)...")
    ...
    def emergency_cleanup(self):
        ...
        if hasattr(self, 'test_data') and self.test_data:
            ...
            if total_collections > 0:
                logger.warning(f"🚨 Emergency cleanup: Removing {total_collections} test collections")
                try:
    ...
    def print_test_summary(self):
        ...
        if summary['failed'] > 0:
            ...
            logger.info(f"\n🔍 Cleanup Strategy:")
            logger.info(f"   ✅ Data from {summary['passed']} passed tests will be cleaned")
            logger.info(f"   🔍 Data from {summary['failed']} failed tests will be preserved for debugging")
        else:
            logger.info(f"\n🎉 All tests passed!")
            logger.info(f"   ✅ All test data will be cleaned up")
    ...
    def force_cleanup_all(self):
        """Force cleanup of all data (for manual cleanup scenarios)"""
        logger.warning("🧹 FORCED CLEANUP: Removing all test data regardless of test results")
